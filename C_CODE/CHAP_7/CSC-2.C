...
int main(int argc, char *argv[])
{
	...
	{
		// char argcopy[strlen(argv[1]) + 3];  VLAs are not allowed in C89
		char *argcopy;
		int i;

		argcopy = malloc(strlen(argv[1]) + 3);

		if(argcopy == NULL)
		{
			printf("ERROR: Memory allocation failed! Program exiting...");
			return 255;
		}

		/* this is utterly retarded... why are you copying using for loop when you can do strcpy?
		for(i = 0; *(argv[1] + i); i++)
			argcopy[i] = *(argv[1] + i);
		*/
