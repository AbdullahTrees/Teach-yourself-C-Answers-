	...
	switch(getche())
	{
		case '1': // Enter
		{
			...
			for(i = 0; i < MAX_NO_BOOKS; i++)
			{
				...
				// Test results: It does work but only when you press enter immediately (i.e. ""), blank spaces will not work i.e. "   "
		...
		case '2': // Search by Author
		{
			...
			if (!match_found)
				printf("Match not found... Did you spell the name of the author correctly? If so, then this book may not be present in the catalog.");
		}
		break;
		...
		case '3': // Search by Title
		{
			...
			if (!match_found)
				printf("Match not found... Did you spell the name of the book correctly? If so, then this book may not be present in the catalog.");
		}
		break;
