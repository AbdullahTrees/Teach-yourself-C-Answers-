...
int main(int argc, char *argv[])
{
	...
	while (!feof(fpointer)) // NOTE: feof only notices an EOF when another function(like fgetc) encounters EOF first, this is why we do size_of_bytes-- after the loop ends. BECAUSE the loop continued executing even though fgetc() found an EOF
	{
		/*ch = fgetc(fpointer);  */
